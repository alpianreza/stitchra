...
export default function NewGrPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<{ data: PoListItem[] }>(
        `/receiving/purchase-orders?per_page=100&q=${encodeURIComponent(poQuery)}`,
      )
      .then((r) => {
        if (active) setPos(r.data);
      })
      .catch((e: Error) => {
  ...
  useEffect(() => {
    api
      .get<{ data: Warehouse[] }>("/receiving/warehouses")
  ...
  useEffect(() => {
    ...
    api
      .get<{ data: Location[] }>(
        `/receiving/locations?warehouse_id=${warehouseId}`,
      )
      .then((r) => {
        if (active) setLocations(r.data);
      })
      .catch((e: Error) => {
        if (active) setError(e.message);
      });
  ...
  async function selectPo(id: string) {
    ...
    let detail: Po;
    try {
      detail = await api.get<Po>(`/receiving/purchase-orders/${id}`);
    } catch (e) {
      setError(e instanceof Error ? e.message : "PO gagal dimuat.");
      return;
    }
    setPo(detail);
    ...
    for (const l of detail.lines) {
      ...
      init[l.id] = {
        ...
        lot_no: "",
        rolls:
          l.material?.tracking_level === "ROLL"
            ? [
                {
                  roll_no: "",
                  qty_buy: String(remaining > 0 ? remaining : 0),
                  qty_meter_actual: "",
                  lot_no: "",
                },
              ]
    ...
    try {
      const lines = po.lines
        ...
        .map((l) => ({
          ...
          lot_no: lineInputs[l.id].lot_no || null,
          rolls:
            l.material?.tracking_level === "ROLL"
              ? lineInputs[l.id].rolls
                  .filter((r) => r.roll_no.trim())
                  .map((r) => ({
                    roll_no: r.roll_no.trim(),
                    qty_buy: Number(r.qty_buy),
                    qty_meter_actual: r.qty_meter_actual
                      ? Number(r.qty_meter_actual)
                      : undefined,
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      ...
      <label className="block text-sm">
        <span className="mb-1 block font-medium">Cari nomor PO</span>
        <input
          value={poQuery}
          onChange={(e) => setPoQuery(e.target.value)}
          placeholder="Cari PO yang belum fully received…"
          className={input}
        />
      </label>
      <section className="grid grid-cols-2 gap-3 rounded-xl border bg-white p-4 md:grid-cols-5">
        <label className="text-sm">
          ...
          >
            ...
            {po && !pos.some((p) => p.id === po.id) && (
              <option value={po.id}>{po.doc_no}</option>
            )}
            {pos.map((p) => (
              <option key={p.id} value={p.id}>
                {p.doc_no} — {p.supplier?.name}
              </option>
            ))}
          </select>
        </label>
        <label className="text-sm">
          <span className="mb-1 block font-medium">Gudang (RM/Trim) *</span>
          <select
            value={warehouseId}
        ...
        <label className="text-sm">
          <span className="mb-1 block font-medium">Lokasi penerimaan</span>
          <select
            value={locationId}
            ...
            className={input}
          >
            <option value="">Tanpa lokasi (staging)</option>
            {locations.map((l) => (
      ...
      {po && (
        <section className="space-y-3 rounded-xl border bg-white p-4">
          <h2 className="font-semibold">
            Lines PO {po.doc_no} — {po.supplier?.name}
          </h2>
          {po.lines.map((l) => {
                ...
                <div className="grid grid-cols-6 items-end gap-2">
                  <div className="col-span-2 text-sm">
                    ...
                    <div className="text-xs text-slate-500">
                      order {Number(l.qty)} · diterima {Number(l.received_qty)}{" "}
                      · sisa {remaining}
                      {isRoll && (
                  ...
                  <label className="text-xs">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLineInputs({
                          ...lineInputs,
                          [l.id]: { ...li, qty_received: e.target.value },
                        })
                      }
                      className={input}
                    />
                      ...
                      {li?.rolls.map((r, ri) => (
                        <div key={ri} className="mb-1 grid grid-cols-4 gap-1">
                          <input
                            ...
                            value={r.roll_no}
                            onChange={(e) =>
                              setRoll(l.id, ri, "roll_no", e.target.value)
                            }
                          ...
                          <input
                            type="number"
                            step="any"
                            placeholder="Qty beli (kg/yard) *"
                            value={r.qty_buy}
                            onChange={(e) =>
                              setRoll(l.id, ri, "qty_buy", e.target.value)
                            }
                          ...
                          <input
                            type="number"
                            step="any"
                            placeholder="Meter aktual"
                            value={r.qty_meter_actual}
                            onChange={(e) =>
                              setRoll(
                                l.id,
                                ri,
                          ...
                          <input
                            ...
                            value={r.lot_no}
                            onChange={(e) =>
                              setRoll(l.id, ri, "lot_no", e.target.value)
                            }
                            className="rounded border px-2 py-1"
                          />
                        </div>
