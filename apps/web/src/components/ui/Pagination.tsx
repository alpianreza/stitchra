...
export function Pagination({ page, totalPages, totalRecords, onPageChange }: PaginationProps) {
  ...
  return (
    <nav
      ...
      className="flex flex-col gap-2 border-t border-[var(--color-border-subtle)] px-3 py-3 sm:flex-row sm:items-center sm:justify-between"
    >
      <p aria-live="polite" className="text-xs text-[var(--color-text-muted)]">
        Halaman <strong className="font-semibold text-[var(--color-text)]">{page}</strong> dari {Math.max(1, totalPages)}
