...
function iteration16InventoryFixture(): array
{
    ...
    $uom = Uom::create([
        'company_id' => 1,
        'code' => 'I16-'.substr(uniqid(), -5),
        'name' => 'Each',
    ]);
    $warehouse = Warehouse::create([
        'company_id' => 1,
        'code' => 'I16-WH-'.substr(uniqid(), -5),
        'name' => 'Iteration 16 Warehouse',
        'type' => 'RM',
        'is_active' => true,
    ]);
...
it('returns the original ITS movement for an identical deterministic replay', function () {
    ...
    $header = [
        'company_id' => 1,
        'source_document_type' => 'iteration_16_tests',
        'source_document_id' => 16001,
    ];
    $lines = [[
        ...
        'uom_id' => $uom->id,
        'unit_cost' => 10,
        'source_document_line_id' => 1,
    ]];

    $first = $its->post('OPENING', $header, $lines, $user);
    $replay = $its->post('OPENING', $header, $lines, $user);

    $balance = StockBalance::withoutGlobalScopes()->where('material_id', $material->id)
...
it('blocks manual journal posting for an inactive company without creating a journal', function () {
    ...
    $debit = ChartOfAccount::create([
        'company_id' => 1,
        'code' => 'I16-DR-'.substr(uniqid(), -4),
        'name' => 'Iteration 16 Debit',
        'type' => 'ASSET',
    ...
    $credit = ChartOfAccount::create([
        'company_id' => 1,
        'code' => 'I16-CR-'.substr(uniqid(), -4),
        'name' => 'Iteration 16 Credit',
        'type' => 'REVENUE',
    ...
    expect(fn () => app(JournalService::class)->post(1, [
        ...
        ['coa_id' => $debit->id, 'debit' => 100],
        ['coa_id' => $credit->id, 'credit' => 100],
...
it('keeps existing authority and regression coverage explicit for Iteration 16', function () {
    $files = [
        'OperationalConvergenceTest.php',
        'PackingShipmentTest.php',
        'FgWarehouseShipmentTraceabilityTest.php',
        'OperationalGlPostingTest.php',
        'JournalServiceTest.php',
        'ProductionOutputAuthorityTest.php',
        'ActualCostTraceabilityTest.php',
        'WipFgCogsBoundaryTest.php',
        'PermissionMiddlewareTest.php',
    ];
    foreach ($files as $file) {
        expect(file_exists(base_path('tests/Feature/'.$file)))->toBeTrue();
    }
});
