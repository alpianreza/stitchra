...
function rollBalanceTotals(int $materialId, int $warehouseId): array
{
    ...
    return [
        (float) $balances->sum(fn ($balance) => (float) $balance->on_hand),
        (float) $balances->sum(fn ($balance) => (float) $balance->quality_hold),
        (float) $balances->sum(fn ($balance) => $balance->available()),
    ];
}
...
test('BR-052: GR fabric tanpa roll ditolak', function () {
    ...
    ], [['po_line_id' => $po->lines->first()->id, 'qty_received' => 100]], $user);
})->throws(RuntimeException::class);

test('BR-002/004/052: GR fabric tersimpan dan diposting per roll ke quality hold', function () {
    [$user, $fabric, $uomKg, , , $warehouse, $po] = grFixture();
    app(ReceivingService::class)->createAndPost(1, [
        'purchase_order_id' => $po->id, 'warehouse_id' => $warehouse->id, 'received_date' => now()->toDateString(),
    ], [[
    ...
    expect($rolls)->toHaveCount(2)
        ->and($rolls[0]->status)->toBe('QUALITY_HOLD')
        ->and((float) $rolls[0]->qty_remaining_meter)->toBe(220.0)
    ...
    expect($onHand)->toBe(100.0)->and($hold)->toBe(100.0)->and($available)->toBe(0.0)
        ->and($po->fresh()->status)->toBe('RECEIVED')
        ->and((float) $po->fresh()->lines->first()->received_qty)->toBe(100.0);
});
...
test('BR-051: partial receiving membuat PO PARTIAL_RECEIVED', function () {
    ...
    app(ReceivingService::class)->createAndPost(1, [
        ...
        'po_line_id' => $po->lines->first()->id, 'qty_received' => 40,
        'rolls' => [['roll_no' => 'R010', 'qty_buy' => 40, 'qty_meter_actual' => 148]],
    ]], $user);
    expect($po->fresh()->status)->toBe('PARTIAL_RECEIVED');
});

test('BR-004: QC per roll release PASS dan return FAIL dari balance roll yang tepat', function () {
    ...
    $inspection = $qc->create(1, $gr, [
        ['gr_line_id' => $grLine->id, 'roll_id' => $rolls[0]->id, 'result' => 'PASS'],
        ['gr_line_id' => $grLine->id, 'roll_id' => $rolls[1]->id, 'result' => 'FAIL'],
    ], $user);
    $qc->finalize($inspection, [], $user);

    [$onHand, $hold, $available] = rollBalanceTotals($fabric->id, $warehouse->id);
    expect($onHand)->toBe(100.0)->and($hold)->toBe(40.0)->and($available)->toBe(60.0)
        ->and($rolls[0]->fresh()->status)->toBe('RELEASED')
        ->and($rolls[1]->fresh()->status)->toBe('REJECTED_RETURNED')
        ->and($grLine->fresh()->status)->toBe('PARTIAL');

    $qc->returnGoods(1, $gr, [[
        'gr_line_id' => $grLine->id, 'roll_id' => $rolls[1]->id,
    ]], 'Shade deviation berat', $user);

    [$onHand, $hold, $available] = rollBalanceTotals($fabric->id, $warehouse->id);
    expect($onHand)->toBe(60.0)->and($hold)->toBe(0.0)->and($available)->toBe(60.0);
});
...
test('BR-050: 3-way match membedakan harga sesuai dan di luar tolerance', function () {
    ...
    $makeInvoice = function (string $docNo, float $price) use ($user, $supplier, $po): SupplierInvoice {
        $invoice = SupplierInvoice::create([
            'company_id' => 1, 'doc_no' => $docNo, 'supplier_id' => $supplier->id,
            'purchase_order_id' => $po->id, 'invoice_date' => now()->toDateString(),
            'total_amount' => 100 * $price, 'created_by' => $user->id,
        ]);
        $invoice->lines()->create([
            'po_line_id' => $po->lines->first()->id, 'qty' => 100,
            'unit_price' => $price, 'amount' => 100 * $price,
        ]);
        return $invoice;
    };

    $matcher = app(ThreeWayMatchService::class);
    expect($matcher->match($makeInvoice('INV-TEST-1', 12), 2, 2)->match_status)->toBe('MATCHED')
        ->and($matcher->match($makeInvoice('INV-TEST-2', 13), 2, 2)->match_status)->toBe('MISMATCH');
});
