...
test('CRUD customer lengkap dengan permission yang benar', function () {
    $user = masterUser([
        'master.customer.view', 'master.customer.create',
...
test('tanpa permission → 403 (BR-110 server-side)', function () {
    ...
    $this->actingAs($user)->postJson('/api/master/customers', ['code' => 'X', 'name' => 'X'])
        ->assertForbidden();
});

test('kode duplikat per company ditolak (validasi unik)', function () {
    $user = masterUser(['master.customer.create']);
    Customer::create(['company_id' => 1, 'code' => 'DUP', 'name' => 'Satu']);

    $this->actingAs($user)->postJson('/api/master/customers', ['code' => 'DUP', 'name' => 'Dua'])
        ->assertUnprocessable();
});
...
test('validasi master: supplier type harus terkontrol', function () {
    ...
    $this->actingAs($user)->postJson('/api/master/suppliers', [
        'code' => 'S1', 'name' => 'S', 'type' => 'NGASAL',
    ])->assertUnprocessable();

    $this->actingAs($user)->postJson('/api/master/suppliers', [
        'code' => 'S1', 'name' => 'S', 'type' => 'FABRIC',
    ])->assertCreated();
});
