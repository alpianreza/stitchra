...
class InwardQcService
{
    ...
    public function create(int $companyId, GoodsReceipt $gr, array $lines, User $user): InwardInspection
    {
        ...
        if ((int) $gr->company_id !== $companyId || $gr->status !== 'POSTED') {
            throw new RuntimeException('Inspeksi hanya dapat dibuat untuk GR POSTED pada company aktif.');
        }
        ...
        return DB::transaction(function () use ($companyId, $gr, $lines, $user): InwardInspection {
            ...
            foreach ($lines as $line) {
                ...
                if ($grLine === null) {
                    throw new RuntimeException('Inspection line tidak berasal dari GR yang dipilih.');
                }

                $rollId = ! empty($line['roll_id']) ? (int) $line['roll_id'] : null;
                if ($grLine->rolls()->exists()) {
                    if ($rollId === null || ! $grLine->rolls()->whereKey($rollId)->exists()) {
                        throw new RuntimeException('Fabric inspection wajib menunjuk roll pada GR line yang sama.');
                    }
                } elseif ($rollId !== null) {
                    throw new RuntimeException('Roll tidak valid untuk GR line non-roll.');
                }

                $key = $grLine->id.':'.($rollId ?? 'none');
                if (isset($seen[$key])) {
                    throw new RuntimeException('Inspection line duplikat.');
                }
                $seen[$key] = true;
    ...
    public function finalize(InwardInspection $inspection, array $ignoredClientLines, User $user): void
    {
        DB::transaction(function () use ($inspection, $user): void {
            ...
            $gr = GoodsReceipt::withoutGlobalScopes()
                ...
                ->whereKey($locked->goods_receipt_id)
                ->firstOrFail();
            ...
            foreach ($locked->lines()->lockForUpdate()->get() as $inspectionLine) {
                $grLine = GrLine::query()
                    ...
                    ->whereKey($inspectionLine->gr_line_id)
                    ->lockForUpdate()
                    ->firstOrFail();
                $affectedGrLines[$grLine->id] = true;

                $roll = null;
                $qty = (float) $grLine->qty_received;
                $lotNo = null;
                if ($inspectionLine->roll_id !== null) {
                    $roll = FabricRoll::withoutGlobalScopes()
                        ->where('gr_line_id', $grLine->id)
                        ->whereKey($inspectionLine->roll_id)
                        ->lockForUpdate()
                        ->firstOrFail();
                    $qty = (float) $roll->qty_buy;
                    $lotNo = $roll->lot_no;
                }
                ...
                if ($inspectionLine->result === 'PASS') {
                    ...
                    $roll?->update(['status' => 'REJECTED_RETURNED']);
                }
            }

            foreach (array_keys($affectedGrLines) as $grLineId) {
                ...
                $status = $statuses->count() === 1
                    ? ($statuses->first() === 'RELEASED' ? 'RELEASED' : 'REJECTED_RETURNED')
                    : 'PARTIAL';
    ...
    {
        if ($returnLines === []) {
            throw new RuntimeException('Supplier return wajib punya minimal 1 line.');
        }

        DB::transaction(function () use ($companyId, $gr, $returnLines, $reason, $user): void {
            if ((int) $gr->company_id !== $companyId || $gr->status !== 'POSTED') {
                throw new RuntimeException('Supplier return harus berasal dari GR POSTED pada company aktif.');
            }
            ...
            foreach ($returnLines as $input) {
                ...
                if ($grLine === null) {
                    throw new RuntimeException('Return line tidak berasal dari GR yang dipilih.');
                }
                $roll = null;
                $qty = (float) $grLine->qty_received;
                $lotNo = null;
                if (! empty($input['roll_id'])) {
                    ...
                    if ($roll === null || $roll->status !== 'REJECTED_RETURNED') {
                        throw new RuntimeException('Hanya roll rejected dari GR line yang sama yang dapat diretur.');
                    }
                    $qty = (float) $roll->qty_buy;
                    $lotNo = $roll->lot_no;
                }
