...
class ShipmentController extends Controller
{
    ...
    public function ship(Request $request,Shipment $shipment):JsonResponse{$company=CurrentCompany::id();$data=$request->validate(['fg_warehouse_id'=>['required','integer',Rule::exists('warehouses','id')->where(fn($q)=>$q->where('company_id',$company)->where('type','FG'))]]);return $this->domain(fn()=>response()->json($this->service->ship($shipment,(int)$data['fg_warehouse_id'],$request->user())));}
    public function show(Request $request,Shipment $shipment):JsonResponse{return response()->json($shipment->load('lines','salesOrder.customer','packingList.cartons'));}
