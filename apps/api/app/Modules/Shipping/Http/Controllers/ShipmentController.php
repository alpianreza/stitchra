...
{
    public function __construct(private ShipmentService $service,private ShipmentInventoryValuationService $valuation,private ShippingPlanService $plans){}
    ...
    {$data=$request->validate(['status'=>['nullable',Rule::in(Shipment::STATUSES)],'per_page'=>'nullable|integer|min:1|max:100']);$query=Shipment::with('salesOrder.customer','packingList','deliverySchedule');if(!empty($data['status']))$query->where('status',$data['status']);return response()->json($query->orderByDesc('id')->paginate($data['per_page']??25));}
    public function store(Request $request,PackingList $packingList):JsonResponse
    {$data=$request->validate(['ship_date'=>'required|date','forwarder'=>'nullable|string|max:255','booking_no'=>'nullable|string|max:64','container_no'=>'nullable|string|max:64','vessel_flight'=>'nullable|string|max:64','port_of_loading'=>'nullable|string|max:64','port_of_discharge'=>'nullable|string|max:64']);return $this->domain(fn()=>response()->json($this->plans->createShipment($packingList,$data,$request->user()),201));}
    ...
    public function show(Request $request,Shipment $shipment):JsonResponse{return response()->json($shipment->load('lines','salesOrder.customer','packingList.cartons.lines','packingList.productionOrder','packingList.qcInspection','deliverySchedule'));}
