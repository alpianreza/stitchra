...
class OperationalIntegrityService
{
    ...
    public function inspect(ProductionOrder $productionOrder, User $user): array
    {
        return DB::transaction(function () use ($productionOrder, $user): array {
            ...
            $markers = DB::table('marker_logs as marker')->join('cut_orders as cut', 'cut.id', '=', 'marker.cut_order_id')
                ->where('cut.company_id', $mo->company_id)->where('cut.production_order_id', $mo->id)->orderBy('marker.id')
                ->get(['marker.id', 'marker.cut_order_id', 'marker.roll_id', 'marker.qty_fabric_used_use', 'marker.qty_fabric_used_m', 'marker.created_at']);
            $layRolls = DB::table('lay_rolls as line')->join('lays as lay', 'lay.id', '=', 'line.lay_id')
                ->join('cut_orders as cut', 'cut.id', '=', 'lay.cut_order_id')->where('cut.company_id', $mo->company_id)
                ->where('cut.production_order_id', $mo->id)->orderBy('line.id')
            ...
            $hasMarkers = $markers->isNotEmpty(); $hasLayRolls = $layRolls->isNotEmpty(); $mixed = $hasMarkers && $hasLayRolls;
            $dispatch = DB::table('fabric_dispatch_balances')->where('company_id', $mo->company_id)->where('production_order_id', $mo->id)
                ->orderBy('id')->get(['id', 'roll_id', 'warehouse_id', 'uom_id', 'qty_dispatched', 'qty_consumed', 'qty_returned']);
            $issues = DB::table('material_issues')->where('company_id', $mo->company_id)->where('production_order_id', $mo->id)
                ->orderBy('id')->get(['id', 'doc_no', 'mode', 'status', 'warehouse_id']);
            $issueIds = $issues->pluck('id');
            $issueLines = $issueIds->isEmpty() ? collect() : DB::table('material_issue_lines')->whereIn('material_issue_id', $issueIds)
                ->orderBy('id')->get(['id', 'material_issue_id', 'material_id', 'roll_id', 'qty', 'uom_id']);
            ...
            $packing = DB::table('packing_lists')->where('company_id', $mo->company_id)->where('production_order_id', $mo->id)
                ->orderBy('id')->get(['id', 'doc_no', 'qc_inspection_id', 'status']);
            $packingIds = $packing->pluck('id');
            $shipments = $packingIds->isEmpty() ? collect() : DB::table('shipments')->where('company_id', $mo->company_id)
                ->whereIn('packing_list_id', $packingIds)->orderBy('id')->get(['id', 'doc_no', 'packing_list_id', 'status']);
            $sourceMovements = $this->sourceMovementEvidence((int) $mo->company_id, $issueIds, $packingIds, $shipments->pluck('id'));
            $duplicateMovements = $sourceMovements->groupBy(fn ($row) => implode('|', [$row->movement_type, $row->source_document_type, $row->source_document_id]))
            ...
            return [
                ...
                'authority_conflict' => ['domain' => 'FABRIC_CONSUMPTION', 'marker_present' => $hasMarkers, 'lay_roll_present' => $hasLayRolls,
                    'mixed_path' => $mixed, 'status' => $mixed ? 'CONFLICT' : ($hasMarkers ? 'LEGACY' : ($hasLayRolls ? 'PARTIAL' : 'NOT DEFINED')),
                    'new_mixed_execution' => 'BLOCKED', 'historical_mutation' => false, 'authority_resolution' => 'DECISION REQUIRED',
                ...
                'consumption_evidence' => ['markers' => $markers->map(fn ($row) => (array) $row)->values(), 'lay_rolls' => $layRolls->map(fn ($row) => (array) $row)->values(), 'dispatch_balances' => $dispatch->map(fn ($row) => (array) $row)->values()],
                'qty_produced' => ['stored_value' => (float) $mo->qty_produced, 'classification' => 'LEGACY COMPATIBILITY FALLBACK — NOT AUTHORITATIVE', 'writer' => 'NOT FOUND', 'production_output_authority' => 'NOT DEFINED',
                    'consumers' => [
                ...
                'backflush' => ['endpoint' => 'POST production/orders/{productionOrder}/issues/backflush', 'status' => 'LEGACY', 'uses_qty_produced' => true,
                    'writes_inventory_through' => 'MaterialIssue → ITS MATERIAL_ISSUE', 'bypasses_its' => false, 'bypasses_reservation' => false,
                    'actual_backflush_overlap' => $issueModesByMaterial->contains(fn ($row) => $row['actual_backflush_overlap']), 'material_evidence' => $issueModesByMaterial,
                    'convergence' => 'BLOCKED — authoritative production output and ACTUAL-vs-BACKFLUSH precedence are not locked'],
                'inventory' => ['authority' => 'ITS', 'source_movements' => $sourceMovements->map(fn ($row) => (array) $row)->values(),
                    'duplicate_source_movements' => $duplicateMovements, 'duplicate_detected' => $duplicateMovements->isNotEmpty(),
                ...
                'operational_chain' => ['cutting_path' => $path, 'cut_outputs' => $cutOutputs, 'bundles' => $bundles, 'production_scans' => $scans, 'wip_transfers' => $wip,
                ...
                'accounting' => ['authority' => 'EXISTING GL / JOURNALS', 'gr_posting' => 'Explicit GR POSTED + one ITS PURCHASE_RECEIPT → deterministic GR_RECEIPT posting_key',
                    'accounting_posting_conflict' => 'NOT DETECTED FOR GR POSTING', 'production_events' => 'BLOCKED where valuation/COGS authority is NOT DEFINED'],
                'legacy_endpoints' => $this->legacyEndpoints(),
                'lineage' => ['forward' => "MO → Cutting → {$path} → Cut Output/legacy Bundle → Sewing → WIP → Finishing → QC → Packing → FG → Shipment → ITS",
    ...
    {
        if ($issueIds->isEmpty() && $packingIds->isEmpty() && $shipmentIds->isEmpty()) return collect();
