...
class MaterialIssueService
{
    ...
    public function issue(ProductionOrder $mo, int $warehouseId, array $lines, User $user): MaterialIssue
    {
        ...
        return DB::transaction(function () use ($mo, $warehouseId, $lines, $user): MaterialIssue {
            ...
            foreach ($lines as $line) {
                ...
                if ($rollId && FabricReturn::withoutGlobalScopes()->where('production_order_id', $locked->id)->where('roll_id', $rollId)->exists()) {
                    throw new RuntimeException('Roll yang sudah ditutup dengan leftover return tidak dapat di-issue ulang ke MO yang sama.');
                }
                ...
                $query = StockReservation::withoutGlobalScopes()
                    ...
                    ->where('mo_id', $locked->id)
                    ->where('warehouse_id', $warehouseId)
                    ->where('material_id', $material->id)
                    ->whereIn('status', ['ACTIVE', 'PARTIAL_ISSUED']);
                if ($rollId) $query->where('roll_id', $rollId);
                ...
                if ($reservation->remaining() + 0.0001 < $qty) throw new RuntimeException("BR-060: issue {$qty} melebihi sisa reservasi {$reservation->remaining()} untuk material #{$material->id}.");
                if ($rollId) {
                    $roll = FabricRoll::withoutGlobalScopes()->where('company_id', $locked->company_id)->where('material_id', $material->id)->whereKey($rollId)->lockForUpdate()->first();
                    if (! $roll || $roll->status !== 'RELEASED') throw new RuntimeException('Roll issue tidak valid atau belum RELEASED.');
                }
                $uomId = $material->isRollTracked() ? (int) $material->use_uom_id : (int) ($line['uom_id'] ?? 0);
                if (! $uomId || (int) ($line['uom_id'] ?? $uomId) !== $uomId) throw new RuntimeException('UOM issue tidak sesuai UOM stok material.');
                $resolved[] = compact('qty', 'material', 'reservation', 'uomId');
            }

            $issue = MaterialIssue::create([
                'company_id' => $locked->company_id,
                'doc_no' => $this->numbering->next($locked->company_id, 'MI'),
            ...
            foreach ($resolved as $item) {
                ...
                $unitCost = $this->companyMovingAverageCost((int) $locked->company_id, $warehouseId, $reservation);
                if ($unitCost !== null) $itsLine['unit_cost'] = $unitCost;
                $itsLines[] = $itsLine;
    ...
    public function backflush(ProductionOrder $mo, int $warehouseId, User $user): ?MaterialIssue
    {
        return DB::transaction(function () use ($mo, $warehouseId, $user): ?MaterialIssue {
            $locked = ProductionOrder::withoutGlobalScopes()->with('bomVersion.lines')->whereKey($mo->id)->lockForUpdate()->firstOrFail();
            $this->assertAccess($locked, $warehouseId, $user);
            if (! in_array($locked->status, ['RELEASED', 'CUTTING', 'SEWING', 'FINISHING'], true)) throw new RuntimeException('Status MO tidak mengizinkan backflush.');
            $produced = (float) $locked->qty_produced;
            if ($produced <= 0) throw new RuntimeException('Backflush memerlukan qty_produced > 0 di MO.');
            $targets = [];
            foreach ($locked->bomVersion->lines->where('is_backflush', true) as $bomLine) {
                ...
                $targets[$bomLine->material_id]['qty'] += $bomLine->grossPerPcs() * $produced;
            }
            ...
            foreach ($targets as $materialId => $target) {
                $already = (float) DB::table('material_issue_lines')
                    ...
                    ->where('material_issue_lines.material_id', $materialId)
                    ->sum('material_issue_lines.qty');
                $remaining = round($target['qty'] - $already, 4);
                ...
                foreach ($reservations as $reservation) {
                    ...
                    $qty = min($reservation->remaining(), $remaining);
                    if ($qty <= 0) continue;
                    $resolved[] = compact('materialId', 'target', 'reservation', 'qty');
                    $remaining = round($remaining - $qty, 4);
                }
                if ($remaining > 0.0001) throw new RuntimeException("BR-060: reservation backflush tidak cukup untuk material #{$materialId}.");
            }
    ...
    public function returnLeftover(ProductionOrder $mo, FabricRoll $roll, int $warehouseId, User $user, ?float $inputQty = null, ?int $inputUomId = null, ?string $reason = null): FabricReturn
    {
        return DB::transaction(function () use ($mo, $roll, $warehouseId, $user, $inputQty, $inputUomId, $reason): FabricReturn {
            ...
            if (! $dispatch) throw new RuntimeException('Roll belum pernah di-dispatch ke MO ini.');
            if ((int) $dispatch->warehouse_id !== $warehouseId) throw new RuntimeException('Return wajib ke warehouse asal dispatch.');
            $available = round((float) $dispatch->qty_dispatched - (float) $dispatch->qty_consumed - (float) $dispatch->qty_returned, 4);
            if ($available <= 0) throw new RuntimeException('Tidak ada leftover dispatched yang dapat dikembalikan.');
            $uomId = (int) $dispatch->uom_id;
            $qtyUse = $inputQty === null ? $available : $this->uoms->convert((int) $lockedMo->company_id, (int) $lockedRoll->material_id, (float) $inputQty, (int) ($inputUomId ?: $uomId), $uomId);
            if (abs($qtyUse - $available) > 0.0001) throw new RuntimeException("Return harus menutup seluruh leftover dispatched ({$available}).");
            if ($qtyUse - $lockedRoll->remainingUse() > 0.0001) throw new RuntimeException('Return melebihi sisa fisik roll.');
            $meters = $this->uoms->toMeters((int) $lockedMo->company_id, $uomId, $qtyUse);
            $return = FabricReturn::create([
                'company_id' => $lockedMo->company_id,
                'doc_no' => $this->numbering->next($lockedMo->company_id, 'MI'),
                'production_order_id' => $lockedMo->id,
                'roll_id' => $lockedRoll->id,
                'warehouse_id' => $warehouseId,
                'uom_id' => $uomId,
                'qty_returned_meter' => $meters,
                'qty_returned_use' => $qtyUse,
                'qty_dispatched_use' => $dispatch->qty_dispatched,
                'qty_consumed_use' => $dispatch->qty_consumed,
                'reason' => $reason,
                'created_by' => $user->id,
            ]);
            $returnLine = [
                'material_id' => $lockedRoll->material_id,
                'warehouse_id' => $warehouseId,
                'roll_id' => $lockedRoll->id,
                'lot_no' => $lockedRoll->lot_no,
                'qty' => $qtyUse,
                'uom_id' => $uomId,
            ];
            $returnCost = $this->unambiguousIssuedUnitCost((int) $lockedMo->company_id, (int) $lockedMo->id, (int) $lockedRoll->id, $warehouseId);
            if ($returnCost !== null) $returnLine['unit_cost'] = $returnCost;
            $this->its->post('PRODUCTION_RETURN', [
                'company_id' => $lockedMo->company_id,
                'source_document_type' => 'fabric_returns',
                'source_document_id' => $return->id,
            ], [$returnLine], $user);
            DB::table('fabric_dispatch_balances')->where('id', $dispatch->id)->update([
                'qty_returned' => (float) $dispatch->qty_returned + $qtyUse,
                'updated_at' => now(),
            ]);
            $this->audit->record('create', $return, after: ['doc_no' => $return->doc_no, 'qty' => $qtyUse, 'uom_id' => $uomId]);
            return $return;
        });
    }
    ...
    private function companyMovingAverageCost(int $companyId, int $warehouseId, StockReservation $reservation): ?float
    {
        ...
        $value = StockBalance::withoutGlobalScopes()->where([
    ...
    private function unambiguousIssuedUnitCost(int $companyId, int $moId, int $rollId, int $warehouseId): ?float
    {
        $rows = StockLedger::withoutGlobalScopes()
            ...
            ->where('material_issues.production_order_id', $moId)
            ->where('stock_ledger.roll_id', $rollId)
            ->where('stock_ledger.warehouse_id', $warehouseId)
            ->where('stock_ledger.ownership', 'COMPANY')
            ->get(['stock_ledger.unit_cost']);
        if ($rows->isEmpty() || $rows->contains(fn ($row) => $row->unit_cost === null)) return null;
        $costs = $rows->map(fn ($row) => number_format((float) $row->unit_cost, 6, '.', ''))->unique();
        return $costs->count() === 1 ? (float) $costs->first() : null;
    }

    private function recordDispatch(int $companyId, int $moId, int $rollId, int $warehouseId, int $uomId, float $qty): void
    {
        DB::table('fabric_dispatch_balances')->insertOrIgnore([
            'company_id' => $companyId, 'production_order_id' => $moId, 'roll_id' => $rollId,
            'warehouse_id' => $warehouseId, 'uom_id' => $uomId,
            'qty_dispatched' => 0, 'qty_consumed' => 0, 'qty_returned' => 0,
