...
> **Dasar:** FASE 0 v1.0, FASE 1 Business Spec v0.1, MODULE_MAP v0.1, PROCESS_FLOW v0.1, BUSINESS_RULES v0.1, ROLES_PERMISSIONS v0.1, DEC-2026-08-13-01
...
- **Uang** = `DECIMAL(19,4)`; **qty** = `DECIMAL(18,4)`; **rate kurs** = `DECIMAL(18,6)`; persen = `DECIMAL(7,4)`.
- **Status** = `VARCHAR` dengan **CHECK constraint / enum terkontrol** per dokumen (tidak ada string bebas).
...
supplier_invoices ── ap_payments ; finance export ── export_batches
...
| `doc_number_counters` | C | Counter per (company, prefix, tahun); increment dalam transaksi terkunci (SELECT ... FOR UPDATE / sequence) | uq(company_id, prefix, period_year) | BR-010 |
...
| `integration_jobs` | T | Job import/export (type, file, status, error log) | idx(company_id, type, status) | OBD-024/032 |
...
| `suppliers` | M | Supplier: type ENUM(FABRIC,TRIM,PACKAGING,SUBCON), lead time, currency, term | uq(company_id, code); idx(type) | — |
| `employees` | M | Karyawan/operator (NIK, section, line_id, skill, is_operator) | uq(company_id, nik); idx(line_id) | — |
| `styles` | M | Style master (style_no, buyer_style_ref, season, kategori ENUM(WOVEN,KNIT,OTHER), product group, lifecycle ENUM(DEVELOPMENT,ACTIVE,DISCONTINUED)) | uq(company_id, style_no); idx(customer_id) | BR-023 |
...
| `materials` | M | Material: type ENUM(FABRIC,TRIM,PACKAGING), kode, nama; fabric: composition, GSM, width, shrinkage_std, buy_uom, use_uom; class untuk backflush flag | uq(company_id, code); idx(type) | BR-002/041 |
| `material_uom_conversions` | C | Konversi default per material (override per roll di GR) | uq(material_id, uom) | BR-002 |
| `warehouses` | M | Gudang: type ENUM(RM,WIP,FG,TRIM,SUBCON_VIRTUAL) | uq(company_id, code); idx(type) | BR-090 |
...
| `defect_library` | M | Defect code, kategori ENUM(FABRIC,WORKMANSHIP,MEASUREMENT,PACKAGING,OTHER), severity ENUM(CRITICAL,MAJOR,MINOR) | uq(company_id, code) | BR-072 |
...
| Tabel | Class | Purpose | Unique / Index penting | Rule |
|---|---|---|---|---|
| `sales_orders` | T | SO: doc_no, customer_id, buyer_po_no, currency+rate, order_date, ex_factory_date, status ENUM baseline, tolerance_pct (override buyer) | uq(company_id, doc_no); idx(customer_id, status); uq(customer_id, buyer_po_no) | BR-010/021 |
...
| `tech_packs` | M | Dokumen tech pack (file refs) | idx(style_id) | — |
| `samples` | T | Sample: stage ENUM(PROTO,FIT,PP,TOP), version, status approval buyer | uq(company_id, doc_no); idx(style_id, stage) | — |
...
| `bom_versions` | M | Versi BOM: version_no, status ENUM(DRAFT,SUBMITTED,APPROVED,OBSOLETE) | uq(bom_id, version_no) | BR-030 |
...
| `cost_sheet_lines` | T | Komponen: type ENUM(FABRIC,TRIM,CM,OVERHEAD,SUBCON,OTHER), qty, rate, amount | idx(cost_sheet_id) | BR-100 |
...
| `supplier_invoices` | T | Invoice supplier + hasil 3-way match ENUM(MATCHED,MISMATCH,PENDING) | uq(company_id, doc_no); idx(supplier_id) | BR-050 |
...
| `gr_lines` | T | Item, qty terima (UOM beli), status ENUM(QUALITY_HOLD,RELEASED,REJECTED_RETURNED) per line | idx(gr_id); idx(po_line_id) | BR-004 |
...
| `inward_inspections` | T | Inspeksi: doc_no, gr ref, inspector, hasil ENUM(PASS,FAIL,PARTIAL) | uq(company_id, doc_no) | BR-004 |
...
| Tabel | Class | Purpose | Unique / Index penting | Rule |
|---|---|---|---|---|
| `stock_ledger` | L | **Append-only**: movement_type ENUM(OPENING,PURCHASE_RECEIPT,PURCHASE_RETURN,QUALITY_RELEASE,TRANSFER_IN,TRANSFER_OUT,MATERIAL_ISSUE,PRODUCTION_RETURN,PRODUCTION_RECEIPT,ADJUSTMENT,OPNAME_ADJUSTMENT,SUBCON_OUT,SUBCON_IN,SHIPMENT), item_type ENUM(MATERIAL,WIP,FG), material_id/style variant ref, warehouse, location, lot, roll, ownership ENUM(COMPANY,BUYER), qty_in, qty_out, uom, unit_cost, total_cost, source_document_type/id/line_id, running_balance | idx(company_id, material_id, warehouse_id, created_at); idx(source_document); idx(roll_id) | BR-001/005/013 |
...
| `stock_reservations` | T | Hard reservation: mo_id, bom ref, material, qty_reserved, qty_issued, status ENUM(ACTIVE,PARTIAL_ISSUED,FULLY_ISSUED,RELEASED) | idx(mo_id); idx(material_id, warehouse_id, status) | BR-006/060 |
...
| Tabel | Class | Purpose | Unique / Index penting | Rule |
|---|---|---|---|---|
| `production_orders` | T | MO: doc_no, so ref, style, qty, planned start/end, status ENUM(...,RELEASED,CUTTING,SEWING,FINISHING,QC,PACKED,CLOSED) | uq(company_id, doc_no); idx(so_id, status) | BR-060 |
...
| `wip_transfers` / `*_lines` | T | Pindah WIP antar proses: from_stage→to_stage ENUM(CUTTING,SEWING,FINISHING,PACKING), bundle refs | uq(company_id, doc_no); idx(bundle via lines) | BR-064 |
...
| `bundles` | T | Bundle: **barcode unik**, style/color/size, qty, status ENUM(CUT,IN_SEWING,SEWN,FINISHED,QC_PASS,REWORK) | uq(company_id, barcode); idx(cutting_order_id, status) | BR-061 |
...
| `downtime_logs` | T | Downtime: line, kategori ENUM(MACHINE,MATERIAL,POWER,SETTING,OTHER), menit, approval supervisor | idx(line_id, log_date) | BR-063 |
...
| Tabel | Class | Purpose | Unique / Index penting | Rule |
|---|---|---|---|---|
| `inspections` | T | Inspeksi: doc_no, type ENUM(INLINE,ROVING,ENDLINE,FINAL), ref (mo/bundle/shipment lot), lot_size, aql_level, sample_size (dari engine), accept_no, reject_no, result ENUM(PASS,FAIL,REWORK,HOLD) | uq(company_id, doc_no); idx(mo_id, type) | BR-008 |
...
| `dispositions` | T | Disposition: action ENUM(REWORK,REPAIR,REJECT,SECOND_GRADE,SCRAP), qty, approver | idx(ncr_id) | BR-070/071 |
...
| Tabel | Class | Purpose | Unique / Index penting | Rule |
|---|---|---|---|---|
| `packing_instructions` | T | Instruksi per SO: type ENUM(SOLID,RATIO,MIXED), ratio detail | uq(so_id, version) | BR-024 |
...
| `shipments` | T | SHP: doc_no, so refs, etd/eta, container, status ENUM(PLANNED,STUFFING,DEPARTED,ARRIVED,INVOICED,PAID) | uq(company_id, doc_no); idx(status) | BR-083 |
...
| Tabel | Class | Purpose | Unique / Index penting | Rule |
|---|---|---|---|---|
| `job_work_orders` | T | JW: doc_no, subcon supplier, process ENUM(PRINT,EMBROIDERY,WASHING,CMT), mo ref, status | uq(company_id, doc_no); idx(supplier_id) | BR-090 |
...
| `journals` / `journal_lines` | T | Journal operasional (source event ref), balanced check (Σdebit=Σcredit) | uq(company_id, doc_no); idx(period) | BR-101 |
| `accounting_periods` | C | Periode + is_locked | uq(company_id, period) | BR-103 |
| `export_batches` | T | Batch ekspor journal (file, status) | idx(company_id, period) | BR-101 |

---

## 4. STATUS ENUM (TERKONTROL)
...
Enum disimpan sebagai CHECK constraint (atau tabel referensi bila perlu UI-driven); penambahan nilai = perubahan blueprint (versioned).
...
## 7. YANG DIPUTUSKAN SEBELUM MIGRATION DITULIS

| Item | Status |
|---|---|
| Engine DB (rekomendasi: **PostgreSQL** — CHECK, partial index, row lock kuat) | ⏳ DECISION REQUIRED (lihat Roadmap §Tech Stack) |
| ORM/migration tool | ⏳ mengikuti tech stack |
| Kolom i18n untuk label master (OBD-030) | default: kolom `name` + `name_translations` JSON hanya untuk label (bukan data relasional) |

---

## NEXT STEP

`ERP_GARMENT_IMPLEMENTATION_ROADMAP.md` — urutan fase implementasi + keputusan teknis yang tersisa. Setelah roadmap disetujui → review konsistensi seluruh blueprint → baru coding Phase 1.
